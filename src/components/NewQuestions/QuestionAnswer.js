...
const QuestionAnswer = ({answer, choice, total}) =>{
    ...
    const per = answer ? Math.trunc((answer.votes.length/total)*100) : Math.trunc((choice.votes.length/total)*100)
        ...
        <Fragment >
        {
            ...
            ?
                <Segment inverted color="green" tertiary>
                    ...
                    <div>{answer.text}</div>
                    <Progress color='yellow' percent={per} progress style={{margin:'15px'}}/>
